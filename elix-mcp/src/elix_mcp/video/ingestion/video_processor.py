...
from elix_mcp.video.ingestion.functions import extract_text_from_chunk, resize_image 
...
class VideoProcessor: 
    ...
    def _check_if_exists(self, video_path: str) -> bool:
       """
       Checks if the Pixeltable and related views/index for the video index exist
       Returns: 
    ...
    def _create_frames_view(self):
        self.frames_view = pxt.create_view(
            ...
            self.video_table,
            iterator=FrameIterator(video=self.video_table.video, num_frames=settings.SPLIT_FRAMES_COUNT),
            if_exists="ignore"
        ) 
